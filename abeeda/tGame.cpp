    ...
    for(int step = 0; step < totalStepsInSimulation; ++step)
    {
        ...
        for(int i = 0; i < swarmSize; ++i)
        {
            if (!preyDead[i])
            {
				...
				//CJHedit - Have we made the predator invisable? 
				double d = predDists[i];
                
                if (visability == 0 && d < preyVisionRange)
                {
		...
		// CJHedit - Store number of prey close to a predator 
		int killTally = 0;
		double killRange = 5*5;

		// CJHedit - has the predator already been killed?
		if(visability == 0)
		{
			// CJHedit - Loop through the prey and see if any have killed predators
			for(int i = 0; i < swarmSize; ++i)
			{
				if (!preyDead[i])
				{
			...
			if(killTally > killPred)
			{

				double newXPos = gridX, newYPos = gridY;

				if (randDouble < 0.5) newXPos *= -1;
				if (randDouble < 0.5) newYPos *= -1;
				// CJHedit - Boot the predator to the edge of the simulation and make invisable
				visability = 20;
				predX = newXPos;
				predY = newYPos;
			}
